...
{
    internal class ESPLogic : MonoBehaviour
    {
        public void OnGUI()
        {
            ...
            foreach (VRC.Player OtherPlayer in PlayerExtensions.GetAllPlayersExceptMe())
            {
                //In-Game Position
                Vector3 pivotPos = OtherPlayer.transform.position; //Pivot point NOT at the feet, at the center
        ...
        public void DrawBoxESP(Vector3 footpos, Vector3 headpos, Color color) //Rendering the ESP
        {
            ...
            //ESP BOX
            Render.ESPBox(footpos.x - (width / 2), (float)Screen.height - footpos.y - height, width, height, color, 2f);
